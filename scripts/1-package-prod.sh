...
# Creates a deployable prod bundle from local source.
...
APP_ARCHIVE_NAME="evergreen-prod-app-${TIMESTAMP}.tar.gz"
PACKAGE_DIR_NAME="evergreen-prod-package-${TIMESTAMP}"
...
LATEST_PACKAGE_PATH="${BUILD_DIR}/evergreen-prod-package.tar.gz"
ZIP_PATH="${BUILD_DIR}/${PACKAGE_DIR_NAME}.zip"
LATEST_ZIP_PATH="${BUILD_DIR}/evergreen-prod-package.zip"
...
echo -e "${BLUE}======================================================${NC}"
echo -e "${BLUE}   Step 1/3 - PROD Package (Run on local laptop)${NC}"
echo -e "${BLUE}======================================================${NC}"
...
echo -e "${YELLOW}Creating single prod zip:${NC} ${ZIP_PATH}"
(
  cd "${BUILD_DIR}"
...
if command -v sha256sum >/dev/null 2>&1; then
  ...
  echo "[prod-package] WARNING: no sha256 utility found; checksum file not created"
fi
